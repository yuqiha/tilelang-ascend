...
def torch_group_norm(x, gamma, beta, num_groups, epsilon):
    return torch.nn.functional.group_norm(
        input=x, num_groups=num_groups, weight=gamma, bias=beta, eps=epsilon,
    )


def run_group_norm(case_id, N, C, spatial_shape, dtype_str,
                   num_groups, epsilon, value_range, block_S=None):
    ...
    cpg = C // num_groups
    actual_block_S = find_block_S(S, cpg, dtype_str)
    print(f"Case {case_id}: PASSED  "
          f"(N={N}, C={C}, spatial={spatial_dims}, dtype={dtype_str}, "
          f"G={num_groups}, block_S={actual_block_S})")
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("GroupNorm TileLang-Ascend 测试 (4D + [cpg, block_S])")
