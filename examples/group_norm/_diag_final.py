"""
精确诊断：分 3 次运行，每次只输出 2 个值，避免额外 buffer 干扰内存规划。

Run 1: total, total_sq
Run 2: mean_sq_val, var_val
Run 3: std_val (fill 到 1 个 buffer)
"""
import sys, os
...
def make_kernel(target, output_mode):
            ...
            with T.Kernel(block_num, is_npu=True) as (cid, vid):
                ...
                if vid == 0:
                    ...
                    with T.Scope("V"):
                        ...
                        if output_mode == "1":
                            T.tile.fill(out_a, total)
                            T.tile.fill(out_b, total_sq)
                        elif output_mode == "2":
                            T.tile.fill(out_a, mean_sq_val)
                            T.tile.fill(out_b, var_val)
                        elif output_mode == "3":
                            T.tile.fill(out_a, std_val)
...
g_var_val = g_total_sq - g_mean_sq + eps
g_std = g_var_val ** 0.5

labels = {
    "1": ("total(=mean)", g_mean, "total_sq(=E[x²])", g_total_sq),
    "2": ("mean_sq_val", g_mean_sq, "var_val", g_var_val),
    "3": ("std_val", g_std, "std_val(copy)", g_std),
}
la, ga, lb, gb = labels[mode]
print(f"[golden] {la}={ga:.6f}  {lb}={gb:.6f}")
...
for tgt in ["ascendc", "pto"]:
    try:
        ...
        src_path = os.path.join(os.path.dirname(__file__), f"_diag_mode{mode}_{tgt}.cpp")
        ...
        a_unique = len(yk[0, 0, :, :block_S].unique())
        b_unique = len(yk[0, 0, :, block_S:2*block_S].unique())
        print(f"[{tgt:8s}] {la}={a_val:.6f} (golden={ga:.6f}, {a_unique} unique)")
        print(f"           {lb}={b_val:.6f} (golden={gb:.6f}, {b_unique} unique)")
