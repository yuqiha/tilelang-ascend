"""
精确诊断：在 TMULS 后立即输出 total_sq 值，验证 TMULS 是否正确写入。
"""
import sys, os
...
for tgt in ["ascendc", "pto"]:
    try:
        kern = make_kernel(tgt)(N, num_groups, cpg_padded, S_padded, block_S, s_num, eps, cpg, S, dtype_str)
        y = kern(x4, g2, b2)
        torch.npu.synchronize()
        yk = y.float().cpu()
        total_val = yk[0, 0, 0, 0].item()
        total_sq_val = yk[0, 0, 0, block_S].item()
        print(f"[{tgt:8s}] total={total_val:.6f} (golden={g_mean:.6f})")
