"""
精确诊断：输出 total, total_sq, var_val, std_val 的中间值。
每个值 fill 到一个完整的 [cpg, block_S] buffer，然后输出到 y 的不同 tile。
"""
import sys, os
        ...
        def main(
            ...
            with T.Kernel(block_num, is_npu=True) as (cid, vid):
                ...
                if vid == 0:
                    ...
                    # 4 个输出 buffer
                    out_total = T.alloc_ub([cpg, block_S], "float32")
                    out_total_sq = T.alloc_ub([cpg, block_S], "float32")
                    out_var = T.alloc_ub([cpg, block_S], "float32")
                    out_std = T.alloc_ub([cpg, block_S], "float32")
                    ...
                    with T.Scope("V"):
                        ...
                        # fill 4 个完整 buffer
                        T.tile.fill(out_total, total)
                        T.tile.fill(out_total_sq, total_sq)
                        T.tile.fill(out_var, var_val)
                        T.tile.fill(out_std, std_val)
                        T.barrier_all()

                        # 输出到 y 的 2 个 block_S tile
                        T.copy(out_var, y[n, g, 0:cpg, 0:block_S])
                        T.copy(out_std, y[n, g, 0:cpg, block_S:2*block_S])
        return main
    return kernel
...
print(f"[golden] total_sq     = {g_total_sq:.6f}")
...
for tgt in ["ascendc", "pto"]:
    try:
        ...
        yk = y.float().cpu()
        var_val_out = yk[0, 0, 0, 0].item()
        std_val_out = yk[0, 0, 0, block_S].item()
        var_unique = yk[0, 0, :, :block_S].unique()
        std_unique = yk[0, 0, :, block_S:2*block_S].unique()
        print(f"\n[{tgt}]")
        print(f"  var_val   = {var_val_out:.6f}  (golden={g_var_val:.6f}, {len(var_unique)} unique)")
        print(f"  std_val   = {std_val_out:.6f}  (golden={g_std:.6f}, {len(std_unique)} unique)")
